        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service) => (
            <Link
              key={service.slug}
              to={`/${service.slug}`}
              className="group rounded-lg bg-card border border-border p-6 hover:shadow-md hover:border-accent/40 transition-all"
            >
              <service.icon className="w-10 h-10 text-accent mb-4" />
              <h2 className="font-heading font-bold text-lg text-foreground mb-2 group-hover:text-accent transition-colors">{service.shortTitle}</h2>
